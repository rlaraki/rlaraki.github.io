...
class MapChoropleth extends MapPlot {
  ...
  */
   ...
   *
   * @param {number} date_indice The index value in the date array (date_ind is non zero when the user do a resize)
    ...
    Promise.all([this.map_promise, this.data_promise]).then((results) => {
      ...
      var countryShapes = this.map_container.selectAll("path")
        ...
        .on("mouseout", function(d) {
          current_plot.tool.classed('hidden', true);
  ...
  /**
   *
   * @param {container} countryShapes Represents the map_container which contains all the path needed to draw the map
   * @param {Date} date The date at which we want to draw the data
